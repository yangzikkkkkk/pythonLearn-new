...
# Load the data
df = pd.read_excel("/Users/yangzi/Library/CloudStorage/Dropbox/云迹科技-Work/22 UP预售工作/104 新版HDOS+UP的素材整理/modified_五星级酒店的结构.xlsx")

# Forward fill the NaN values
df.fillna(method='ffill', inplace=True)

# Create a separate data frame for each level
df_departments = pd.DataFrame(df['部门'].unique(), columns=['name'])
df_departments['parent'] = ''

df_positions = df[['部门', '岗位']].drop_duplicates()
df_positions.columns = ['parent', 'name']

df_duties = df[['岗位', '主要职责']].drop_duplicates()
df_duties.columns = ['parent', 'name']

# Sort departments starting with "前厅部"
start_department = "前厅部"
sorted_departments = df_departments['name'].tolist()
sorted_departments.sort(key=lambda x: (x != start_department, x))  # this will put "前厅部" at the beginning and sort the rest alphabetically
